...
# Add a selectbox for the user to choose a stock on the sidebar
selected_ticker = st.sidebar.selectbox('Select a stock', list(stock_data_urls.keys()))

# Add date input widgets with default values on the sidebar
start_date = st.sidebar.date_input('Start date', value=pd.to_datetime('2020-01-01'))
end_date = st.sidebar.date_input('End date', value=pd.to_datetime('2021-12-31'))

# Add a number input widget for hypothetical High price on the sidebar
high_price = st.sidebar.number_input('Enter a hypothetical High price (for Today):', value=0.0)
...
st.sidebar.write(f'The predicted Close price (for tomorrow) is: ${predicted_close_price.item():.2f}')
